...
# =============================================================================
# MT-RL Training Script - Paper-Aligned (GE-Lab Table 8) - 4x H200 adaptation
# Multi-Turn RL with trajectory-level rewards
#
# Adapted from mt_rl_448.sh (3x A100 80GB, /ext_hdd2 host) for this machine:
#   - 4x H200 143GB: NPROC=4, num_generations 3->4, grad_accum 16->12
#     (effective batch stays 48 completions/step = 12 prompts x 4 generations)
#   - Base model: namhokaist/STRL_True_Final (HF snapshot; the local
#     st_rl_448/checkpoint-3420 from the original host is the same model)
#   - DATALOADER_NUM_WORKERS=0 (64MB /dev/shm on this container)
#   - wandb optional: logs to wandb only when WANDB_API_KEY is set
# =============================================================================
...
if [ -z "$WANDB_API_KEY" ] && ! grep -qs "api.wandb.ai" ~/.netrc; then
    echo "WARNING: no wandb credential found - running WANDB_MODE=offline (sync later with 'wandb sync')"
    export WANDB_MODE="offline"
fi
export HF_HOME="${HF_HOME:-/workspace/nhkoh/hf_cache}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/workspace/nhkoh/.cache}"
export TORCH_HOME="${TORCH_HOME:-/workspace/nhkoh/.cache/torch}"
export CUDA_HOME=/usr/local/cuda-12.4
export PATH=/opt/conda/bin:$CUDA_HOME/bin:$PATH
...
# STRL_True_Final snapshot (downloaded via huggingface_hub; equals the original
# ST-RL v3 checkpoint ./checkpoint/gui_exp/st_rl_448/v0-20260221_215851/checkpoint-3420)
MODEL_PATH="$(ls -d $HF_HOME/hub/models--namhokaist--STRL_True_Final/snapshots/*/ | head -1)"
export MODEL_PATH
export DATASET_PATH="${DATASET_PATH_OVERRIDE:-datas/mt_rl_aligned.json}"  # 2200 tasks from sub2+sub3
...
export NUM_GENERATIONS=4  # Paper: 8. Must divide total batch (4 GPUs x 1 = 4). Original A100 run used 3.
...
# batch=1, num_gen=4: 1 episode/GPU (55 GiB proven stable on 80GB A100 at num_gen=3)
# 4 GPUs x batch=1 x grad_acc=12 = 48 effective batch (same as original 3x16)
export NPROC_PER_NODE=4
export PER_DEVICE_TRAIN_BATCH_SIZE=1
export GRADIENT_ACCUMULATION_STEPS=12
...
export SAVE_STEPS=50  # was 100; step-71 NCCL stall killed a run with zero checkpoints saved
export SAVE_TOTAL_LIMIT=2
export SAVE_ONLY_MODEL="true"
export LOGGING_STEPS=1
export DATALOADER_NUM_WORKERS=0  # 64MB /dev/shm limit on this container
...
OUTPUT_DIR="${OUTPUT_DIR_OVERRIDE:-$BASE_OUTPUT_DIR/$SAVE_NAME/v0-${time_start}}"
LOG_FILE="$BASE_LOG_DIR/mt_rl_448_${time_start}.log"

echo "============================================================"
echo "MT-RL TRAINING (Paper Table 8 Aligned) - 4x H200"
echo "============================================================"
