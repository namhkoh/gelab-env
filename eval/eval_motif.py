"""
MoTIF benchmark evaluation using Qwen2.5-VL's computer_use guided generation.
...
def _eval_chunk(gpu_id, model_path, samples, result_queue):
    ...
    for sample in tqdm(samples, desc=f"MoTIF:GPU{gpu_id}"):
        img = sample["image"].convert("RGB")
        instruction = sample["instr"]
        screen_bbox = sample["ui_pos_obj_screen_bbox"]
        img_w, img_h = img.size

        if screen_bbox is None or len(screen_bbox) < 4:
            total += 1
            continue

        gt_bbox = [screen_bbox[0]/img_w, screen_bbox[1]/img_h,
                    screen_bbox[2]/img_w, screen_bbox[3]/img_h]
...
def main():
    parser = argparse.ArgumentParser(description="MoTIF eval with Qwen2.5-VL guided generation")
    ...
    print(f"Loading MoTIF dataset...")
    ds = load_dataset("HongxinLi/MOTIF-EVAL", split="test_au_tu",
