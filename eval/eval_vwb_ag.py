"""
VisualWebBench Agent Grounding (VWB-AG) evaluation using Qwen2.5-VL guided generation.

Dataset: HongxinLi/VWB-AG (103 samples, bbox in 0-1 normalized format)
...
def _eval_chunk(gpu_id, model_path, samples, result_queue):
    ...
    for sample in tqdm(samples, desc=f"VWB-AG:GPU{gpu_id}"):
        img = sample["image"].convert("RGB")
        instruction = sample.get("detailed_elem_desc", sample.get("elem_desc", ""))
...
def main():
    parser = argparse.ArgumentParser(description="VWB-AG eval with Qwen2.5-VL guided generation")
    ...
    print("Loading VWB-AG dataset...")
    ds = load_dataset("HongxinLi/VWB-AG", split="test",
